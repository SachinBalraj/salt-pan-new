...
def comparison_rows(db: Session) -> List[dict]:
    ...
    for out in outcomes:
        ...
        snap = _snapshot(pred) if pred else {}
        hit = "na"
        error = None
        projected_yield = None
        pred_score = snap.get("score") if pred else None
        ...
        if pred and isinstance(features, dict) and "max_risk_horizon" in features:
            try:
                risk_score = float(features["max_risk_horizon"])
        ...
        if pred and isinstance(features, dict):
            projected_yield = features.get("projected_yield_kg")
        ...
        rows.append({
            ...
            "prediction_score": float(pred_score) if pred_score is not None else 0.0,
...
def _outcome_to_row(out: HarvestOutcome, pan: Pan, pred: Optional[Prediction],
    ...
    row["action_recorded"] = panel.get("action_taken", "")
    row["harvest_ready_flag"] = 1 if panel.get("action_taken") == "harvest" else 0
    ctx: Dict[str, float] = {}
    if pred:
        ctx = dict((_snapshot(pred).get("features") or {}))
    ...
    row["harvest_readiness"] = 1.0 if panel.get("action_taken") == "harvest" else 0.0
